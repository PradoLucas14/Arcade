...
async function obtenerJuegosDestacados() {
...
async function mostrarJuegosEnTabla(juegos) {
    const tablaJuegos = document.getElementById("tablaJuegos");
    tablaJuegos.innerHTML = "";    

    const juegosDestacados = await obtenerJuegosDestacados();
    ...
    juegos.forEach(juego => {
        ...
        const esDestacado = juegosDestacados.some(destacado => destacado.titulo === juego.titulo);
        const iconoPublicado = juego.publicado ? iconoTrue : iconoFalse;
        const claseDestacado = esDestacado ? 'destacado' : '';

        const fila = `
            <tr class="fila ${claseDestacado}">
...
async function destacarJuego(id) {
    ...
    if (datosJuego) {
        ...
        const juegoSeleccionado = {
            ...
            desarrolladora: datosJuego.desarrollador,
            imagen2: datosJuego.imagen2
        };

        try {
            const response = await axios.post(url, juegoSeleccionado);
            console.log('Juego destacado exitosamente:', response.data);
            alert("Se agrego este juego a destacados")
            return
        } catch (error) {
            console.error('Error al destacar el juego:', error);
            alert("No se pudo agregar este juego a destacados")
        }
    } else {
